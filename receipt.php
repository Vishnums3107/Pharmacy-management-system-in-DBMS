...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		foreach($data as $row) {
			...
			$this->Cell($w[4], 4, number_format(is_numeric($row[4]) ? (float)$row[4] : 0.00, 2), 'LR', 0, 'R', $fill);
			$this->Cell($w[4], 4, number_format(floatval($row[4]), 2), 'LR', 0, 'R', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
require_once('tcpdf/tcpdf.php');
require_once('tcpdf/config/tcpdf_config.php'); // Ensure the config file is included
if (!defined('PDF_FONT_MONOSPACED')) {
    define('PDF_FONT_MONOSPACED', 'courier'); // Default monospaced font
}
